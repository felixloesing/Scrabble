...
Board::Board(std::string board_file_name, std::string board_init_file_name):
_squares(),
_numRows(),
_numColumns(),
_isFirstMove(false)
{
	...
	//place init file configuration
	if (board_init_file_name != "") {
		placeStartTiles(board_init_file_name);
	} else {
		return;
	}
}

void Board::placeStartTiles(std::string board_init_file_name) {
	std::ifstream initFileStream(board_init_file_name);

	if(!initFileStream)
	{
		throw FileException("INIT");
	}
	...
	while(!initFileStream.eof()) {
		std::string r;
		std::getline(initFileStream, r);

		// if the file ends with a newline, we will get a final "phantom" 0-length string
		// so here we expressly ix-nay that.
		if(initFileStream.eof() && r.empty())
		{
			break;
		}

		for (unsigned int i = 0; i < r.length(); i = i + 3)
		{
			//found tile to place
			if (r.at(i) != '.') {
				...
				unsigned int t = (unsigned int) r.at(i+1) - '0';
				unsigned int o = (unsigned int) r.at(i+2) - '0';
				points = t*10 + o;

				Tile* newTile = new Tile(letter, points);
				getSquare((i/3)+1, index + 1)->placeTile(newTile);
			}
		}
		++index;
	}
...
/*	if (x >= 1 && x <= _numColumns && y >= 1 && y <= _numRows) {
		return _squares.at(y - 1).at(x - 1);
	}
	return NULL;*/
}
